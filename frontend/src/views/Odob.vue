<template>
  <el-table :data="tableData.list" v-loading="loading" height="97%" width="100%" :cell-style="{ textAlign: 'left' }"
    ...
    <el-table-column prop="intro" label="简介" width="300">
      <template #default="scope">
        <el-popover title="简介" trigger="hover" placement="top" :width="480" :disabled="scope.row.intro.length <= 30"
    ...
    <el-table-column fixed="right" label="操作" width="200">
      <template #default="scope">
        <el-button icon="VideoPlay" size="small" type="primary" link @click="handlePlay(scope.row)">播放</el-button>
...
const download = async (id: number, dType: number) => {
  await OdobDownload(id, dType).then((info) => {
    console.log(info)
  }).catch((error) => {
    ElMessage({
      message: error,
